...
void SingleFlashAttention(const size_t start_pos, const size_t last_pos,
  ...
  // Fold short blocks into the numerator to bound float addition-chain length
  // for long windows with same-sign values. Both sums share the running max.
  MulByConstTo(/*mul=*/1.0f, v.Row(pos_mod), att_out, v.Cols(), ctx, worker);
  if (start_pos == last_pos) return;
  constexpr size_t kBlockSize = 128;
  HWY_DASSERT(v.Cols() <= kMaxQKVDim);
  HWY_ALIGN float other[kMaxQKVDim];
  std::fill_n(other, v.Cols(), 0.0f);
  for (size_t pos = start_pos + 1; pos <= last_pos; ++pos) {
    ...
    if (x > m) {
      ...
      MulByConst(scale, other, v.Cols());
      MulByConstAndAdd(/*mul=*/1.0f, v.Row(pos_mod), other, v.Cols());
    } else {
      const float weight = std::exp(x - m);
      d += weight;
      MulByConstAndAdd(weight, v.Row(pos_mod), other, v.Cols());
    }
    if ((pos - start_pos) % kBlockSize == 0) {
      MulByConstAndAdd(/*mul=*/1.0f, other, att_out, v.Cols());
      std::fill_n(other, v.Cols(), 0.0f);
    }
  }
  MulByConstAndAdd(/*mul=*/1.0f, other, att_out, v.Cols());
